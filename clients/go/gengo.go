...
func generateInterface(packageName, basePath, outputPath string, s *spec.Swagger, serviceName string, paths *spec.Paths) error {
	...
	moduleName, versionSuffix := utils.ExtractModuleNameAndVersionSuffix(packageName, outputPath)
	g.Printf("%s", swagger.ImportStatements([]string{"context", moduleName + outputPath + "/models" + versionSuffix}))
	g.Printf("//go:generate mockgen -source=$GOFILE -destination=mock_client.go -package client --build_flags=--mod=mod -imports=models=%s%s /models%s\n\n", moduleName, outputPath, versionSuffix)

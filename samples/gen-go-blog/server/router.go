...
import (
	...
	"github.com/Clever/wag/samples/v8/gen-go-blog/servertracing"
...
func newRouter(c Controller) *mux.Router {
	...
	router.Use(servertracing.MuxServerMiddleware("blog"))
...
func AttachMiddleware(router *mux.Router, addr string, m []func(http.Handler) http.Handler, options ...func(*serverConfig)) *Server {
	...
	handler := withMiddleware("blog", router, m, config)

// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// Unathorized unathorized
//
// swagger:model Unathorized
type Unathorized struct {

	// message
	Message string `json:"message,omitempty"`
}

// Validate validates this unathorized
func (m *Unathorized) Validate(formats strfmt.Registry) error {
	return nil
}

// ContextValidate validates this unathorized based on context it is used
func (m *Unathorized) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *Unathorized) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *Unathorized) UnmarshalBinary(b []byte) error {
	var res Unathorized
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}

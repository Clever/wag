...
func (s *Server) Serve() error {
	...
	go func() {
		metrics.Log("wag-patch", 1*time.Minute)
	}()
...
func New(c Controller, addr string) *Server {
	...
	l := logger.New("wag-patch")

	r.Methods("PATCH").Path("/wagpatch").HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		logger.FromContext(r.Context()).AddContext("op", "wagpatch")
